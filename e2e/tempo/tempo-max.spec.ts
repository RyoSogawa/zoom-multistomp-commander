...
test.describe('Tempo Operation', () => {
  test('should change Tempo to maximum 250 BPM', async ({ page }) => {
    await page.goto('/');
    ...
    await expect(page.getByRole('slider')).toHaveValue('250');

    // 5. Verify SysEx output is 'F0 52 00 6E 64 20 00 64 02 7A 01 00 00 00 F7'
    await expect(page.getByText('F0 52 00 6E 64 20 00 64 02 7A 01 00 00 00 F7')).toBeVisible();
  });
});
